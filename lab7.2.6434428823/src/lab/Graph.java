...
public class Graph {
    ...
    private LinearProbingHashMap graph = new LinearProbingHashMap(4);
    private Vertex[] vertexes;
    private Edge[] edges;

    public LinearProbingHashMap getGraph() {
        return graph;
    }
    ...
    public Graph(int capV, int capEd) {
        ...
        for (int i = 0; i < edges.length; i++) {
            ...
            Vertex end = vertexes[rnd.nextInt(vertexes.length)];
            Vertex st = vertexes[rnd.nextInt(end.getName() + 1)];
            ...
            while (contain) {
                contain = false;
                end = vertexes[rnd.nextInt(vertexes.length)];
                st = vertexes[rnd.nextInt(end.getName() + 1)];
    ...
    public Graph(int[][] adjM) {
        ...
        for (int i = 0; i < adjM.length; i++) {
            for (int j = 0; j < i + 1; j++) {
                if (adjM[i][j] != 0) {
                    Edge edge = new Edge(new Vertex(j), new Vertex(i));
                    edges[n] = edge;
    ...
    public int weight(Edge e) {
        ...
        if (graph.containsKey(e)) return (int)graph.get(e);
        if (graph.containsKey(eR)) return (int)graph.get(eR);
        return 0;
    }
